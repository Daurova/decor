// apps/landing/sections/Contact.tsx
'use client';
...
export function Contact() {
  return (
    ...
    >
      ...
      {/* Контент — внизу */}
      <div className="relative z-10 w-full max-w-4xl px-4 pb-12 md:pb-16 text-center">
        <Reveal>
        ...
        <Reveal delay={0.15}>
          <p className="mt-2 text-[#D4C5A9]/90 text-lg md:text-xl font-light tracking-wide drop-shadow-[0_0_30px_rgba(212,197,169,0.2)]">
            Оставьте заявку — мы пришлём коммерческое предложение в течение часа.
          </p>
        </Reveal>

        {/* Контактная информация без карточек */}
        <div className="mt-6 flex flex-col sm:flex-row justify-center items-center gap-4 md:gap-6 text-[#D4C5A9]/80 text-sm md:text-base font-light">
          <a href="tel:+74950000000" className="hover:text-white transition-colors duration-300 flex items-center gap-2">
            <span className="text-lg">📞</span> +7 (495) 000-00-00
          </a>
          <span className="hidden sm:inline text-[#D4C5A9]/20">|</span>
          <a href="mailto:info@gloriter.ru" className="hover:text-white transition-colors duration-300 flex items-center gap-2">
            <span className="text-lg">✉️</span> info@gloriter.ru
          </a>
          <span className="hidden sm:inline text-[#D4C5A9]/20">|</span>
          <span className="flex items-center gap-2">
            <span className="text-lg">📍</span> Москва, ул. Строителей, 15
          </span>
        </div>
        ...
        <Reveal delay={0.3}>
          <motion.button
            ...
            whileTap={{ scale: 0.96 }}
            className="mt-6 px-8 py-3 rounded-full bg-[#D4C5A9]/20 backdrop-blur-sm text-white border border-white/20 hover:bg-[#D4C5A9]/30 transition-all duration-300 text-sm md:text-base font-medium"
          >
            Оставить заявку
          </motion.button>
        </Reveal>
      ...
      >
        <span>От заявки до поставки</span>
